...
import { runPhase3Ingestion } from '../linker/phase3IngestionRunner';

function main() {
  console.log('======================================================');
  console.log('   LINKER PHASE 3 LARGE BATCH INGESTION (10,000)      ');
  console.log('======================================================\n');

  const report = runPhase3Ingestion();

  // Write machine-readable JSON report
  const jsonPath = path.resolve('data/linker_phase3_report.json');
  fs.writeFileSync(jsonPath, JSON.stringify(report, null, 2), 'utf-8');

  // Build human-readable Markdown summary
  const lines: string[] = [
    `# Linker Phase 3 Large Batch Ingestion Report`,
    ``,
  ...
  lines.push(``);
  lines.push(`### Chunk Execution Details (20 Chunks of 500)`);
  lines.push(``);
  ...
  const mdPath = path.resolve('data/linker_phase3_summary.md');
