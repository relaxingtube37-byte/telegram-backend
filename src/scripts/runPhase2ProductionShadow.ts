...
import { buildPhase2Cohort } from '../linker/phase2IngestionRunner';
import type { IncomingRawMatch } from '../linker/types';

export interface Phase2ShadowChunkReport {
  chunkIndex: number;
...
export interface Phase2ShadowExecutionReport {
  timestamp: string;
  ...
  error?: string;
  chunks: Phase2ShadowChunkReport[];
}

export class Phase2ProductionShadowRunner {
  ...
  public async run(): Promise<Phase2ShadowExecutionReport> {
    const snapshotPath = path.join(this.snapshotDir, 'database_wal_safe_pre_phase2.sqlite');
    ...
    // Explicitly configure and verify connection PRAGMAs
    ...
    const chunkReports: Phase2ShadowChunkReport[] = [];
    ...
    try {
      ...
      // 2. Build 2,500-match deterministic high-entropy cohort
      console.log('[Phase 2 Shadow] Building deterministic 2,500-match entropy cohort...');
      const { cohort } = buildPhase2Cohort(this.targetDbPath, liveDb);
      if (cohort.length !== 2500) {
        throw new Error(`Invalid Phase 2 cohort size: expected 2,500, got ${cohort.length}`);
      }

      // Dry-Run mode: validate preconditions without writes
      if (this.isDryRun) {
        console.log('[Phase 2 Shadow] ℹ️ DRY RUN MODE: Prerequisites and cohort validated. Zero writes executed.');
        liveDb.close();
      ...
      // -----------------------------------------------------------------------
      console.log(`[Phase 2 Shadow] Creating pre-phase snapshot: ${snapshotPath}...`);
      ...
      console.log(`[Phase 2 Shadow] ✅ Snapshot verified: ${(snapStat.size / (1024 * 1024)).toFixed(2)} MB`);
      ...
      const chunkSize = 50;
      const totalChunks = Math.ceil(cohort.length / chunkSize); // 50 chunks
      ...
      for (let chunkIdx = 0; chunkIdx < totalChunks; chunkIdx++) {
        ...
        if (rollingBusyCount > 3) {
          console.warn(`[Phase 2 Shadow] ⚠️ High SQLite lock contention (>3 busy in 60s). Pausing for 30s...`);
          ...
          try {
            ...
            if (err.message && (err.message.includes('busy') || err.message.includes('locked'))) {
              ...
              if (totalBusyRetries > 10) {
                throw new Error(`CIRCUIT BREAKER TRIPPED: > 10 SQLITE_BUSY events during Phase 2.`);
              }

              const jitter = Math.floor(Math.random() * 50);
              const backoffMs = retriesForChunk === 1 ? 100 + jitter : retriesForChunk === 2 ? 250 + jitter : 500 + jitter;
              console.warn(`[Phase 2 Shadow] SQLITE_BUSY on chunk ${chunkIdx + 1}/${totalChunks} (attempt ${retriesForChunk}/3). Retrying in ${backoffMs}ms...`);
        ...
        // Sentinel Check 4: Review Queue hard ceiling (> 30%)
        ...
        if (totalSoFar >= 500 && currentQueueRate > 30.0) {
          throw new Error(`HARD STOP: Review queue rate reached ${currentQueueRate.toFixed(1)}% (exceeded 30% hard ceiling).`);
        }

        // Checkpoint WAL passively every 4 chunks (200 matches)
        if ((chunkIdx + 1) % 4 === 0) {
        ...
        if (walSize > 50 * 1024 * 1024) {
          console.warn(`[Phase 2 Shadow] WAL size exceeded 50 MB (${(walSize / 1024 / 1024).toFixed(1)} MB). Checkpointing passively...`);
          liveDb.pragma('wal_checkpoint(PASSIVE)');
        }
        ...
        console.log(
          `[Phase 2 Shadow] Chunk ${chunkIdx + 1}/${totalChunks} committed (${chunkItems.length} matches, +${chunkAutoLinks} links, +${chunkReviewQueues} queue, WAL: ${(walSize / 1024 / 1024).toFixed(2)} MB, ${chunkDuration}ms)`
        );
...
if (require.main === module) {
  ...
  const runner = new Phase2ProductionShadowRunner({
  ...
  runner
    .run()
    .then((report) => {
      const jsonPath = path.resolve('data/linker_phase2_production_shadow_report.json');
      const mdPath = path.resolve('data/linker_phase2_production_shadow_report.md');

      fs.writeFileSync(jsonPath, JSON.stringify(report, null, 2), 'utf-8');

      const md = [
        `# Phase 2 Production Shadow Ingestion Report (${report.mode})`,
        ``,
        ...
        `| :--- | :---: | :---: | :---: |`,
        `| **Cohort Matches** | **${report.totalCohortSize}** | Exactly 2,500 | ✅ PASS |`,
        `| **Chunks Processed** | **${report.chunksCompleted} / ${report.totalChunks}** | 50 chunks (50 matches/chunk) | ✅ PASS |`,
      ...
      console.log('\n======================================================');
      console.log(`PHASE 2 SHADOW RUNNER: ${report.status} (${report.mode})`);
      console.log('======================================================');
    ...
    .catch((err) => {
      console.error('\n[FATAL ERROR IN PHASE 2 SHADOW RUNNER]:', err);
      process.exit(1);
    });
}
