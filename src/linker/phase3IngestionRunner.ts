...
export interface Phase3ChunkResult {
  chunkIndex: number;
...
export interface Phase3IngestionReport {
  timestamp: string;
  targetDbPath: string;
  snapshotPath: string;
...
/**
 * Builds the deterministic 10,000-match Phase 3 cohort with controlled entropy:
 * 1. Standard ATP/WTA Tour-Level Matches (5,000 matches: 3,000 base + 2,000 overlap)
 * 2. Challenger & ITF Matches (2,600 matches: 1,600 base + 1,000 overlap)
 * 3. Cross-Day Overlaps (600 matches: 400 base + 200 shifted +1 day)
 * 4. Tournament Alias Variations (600 matches: 400 base + 200 alias variations)
 * 5. Retirements & Walkovers (400 matches: 200 base + 200 overlap)
 * 6. Sibling Ambiguity & Ambiguous Aliases (400 matches)
 * 7. Adversarial Identity Conflicts (400 matches: 200 base + 200 conflict presentations)
 * Total: exactly 10,000 matches
 */
export function buildPhase3Cohort(
  sourceDbPath: string,
  ...
  try {
    ...
    const bucketCounts: Record<string, number> = {
      ...
      '7. Adversarial Identity Conflicts (Base)': 0,
      '7. Adversarial Identity Conflicts (Conflicts)': 0,
    };

    // -------------------------------------------------------------------------
    // BUCKET 1: Standard ATP/WTA Tour-Level Matches (5,000 matches: 3000 base + 2000 overlap)
    // -------------------------------------------------------------------------
    ...
    const cleanTour = tourMatches.filter(m => !alreadyIngestedIds.has(m.id)).slice(0, 3000);
    if (cleanTour.length < 3000) {
      throw new Error(`Insufficient clean tour matches for Phase 3. Found: ${cleanTour.length}`);
    }
    ...
    for (let i = 0; i < 3000; i++) {
      ...
      cohort.push({
        sourceName: 'sackmann',
        sourceMatchId: `p3_tour_base_${m.id}`,
        ...
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'tour_base', phase: 'phase_3' },
      });
    ...
    for (let i = 0; i < 2000; i++) {
      ...
      cohort.push({
        sourceName: 'pbp',
        sourceMatchId: `p3_tour_overlap_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'tour_overlap', phase: 'phase_3' },
      });
      bucketCounts['1. Tour-Level ATP/WTA (Overlaps)']++;
    }

    // -------------------------------------------------------------------------
    // BUCKET 2: Challenger & ITF Matches (2,600 matches: 1600 base + 1000 overlap)
    // -------------------------------------------------------------------------
    const challMatches = sourceDb.prepare(`
      SELECT id, tour, tourney_name, surface, match_date, round_name, winner_name, loser_name, score
      FROM historical_matches
      WHERE match_date >= '2022-01-01' AND match_date <= '2024-12-31'
    ...
    const cleanChall = challMatches.filter(m => !alreadyIngestedIds.has(m.id)).slice(0, 1600);
    if (cleanChall.length < 1600) {
      throw new Error(`Insufficient clean challenger matches for Phase 3. Found: ${cleanChall.length}`);
    }
    ...
    for (let i = 0; i < 1600; i++) {
      ...
      cohort.push({
        sourceName: 'sackmann',
        sourceMatchId: `p3_chall_base_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'challenger_base', phase: 'phase_3' },
      });
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      cohort.push({
        sourceName: 'pbp',
        sourceMatchId: `p3_chall_overlap_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'challenger_overlap', phase: 'phase_3' },
      });
      bucketCounts['2. Challenger & ITF (Overlaps)']++;
    }

    // -------------------------------------------------------------------------
    // BUCKET 3: Cross-Day Overlaps (600 matches: 400 base + 200 shifted +1 day)
    // -------------------------------------------------------------------------
    const crossDayPool = tourMatches.filter(m => !alreadyIngestedIds.has(m.id)).slice(3000, 3400);
    for (let i = 0; i < 400; i++) {
      const m = crossDayPool[i];
      cohort.push({
        sourceName: 'sackmann',
        sourceMatchId: `p3_cross_base_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'cross_day_base', phase: 'phase_3' },
      });
    ...
    for (let i = 0; i < 200; i++) {
      ...
      cohort.push({
        sourceName: 'pbp',
        sourceMatchId: `p3_cross_shift_${m.id}`,
        matchDate: nextDate,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'cross_day_shifted', phase: 'phase_3' },
      });
      bucketCounts['3. Cross-Day Cases (Shifted)']++;
    }

    // -------------------------------------------------------------------------
    // BUCKET 4: Tournament Alias Variations (600 matches: 400 base + 200 alias variations)
    // -------------------------------------------------------------------------
    const aliasPool = tourMatches.filter(m => !alreadyIngestedIds.has(m.id)).slice(3400, 3800);
    for (let i = 0; i < 400; i++) {
      const m = aliasPool[i];
      cohort.push({
        sourceName: 'sackmann',
        sourceMatchId: `p3_alias_base_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'tournament_alias_base', phase: 'phase_3' },
      });
    ...
    for (let i = 0; i < 200; i++) {
      ...
      cohort.push({
        sourceName: 'pbp',
        sourceMatchId: `p3_alias_var_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'tournament_alias_var', phase: 'phase_3' },
      });
      bucketCounts['4. Tournament Aliases (Variations)']++;
    }

    // -------------------------------------------------------------------------
    // BUCKET 5: Retirements & Walkovers (400 matches: 200 base + 200 overlap)
    // -------------------------------------------------------------------------
    const retPool = sourceDb.prepare(`
      SELECT id, tour, tourney_name, surface, match_date, round_name, winner_name, loser_name, score
      FROM historical_matches
      WHERE (score LIKE '%RET%' OR score LIKE '%W/O%')
        AND winner_name IS NOT NULL AND loser_name IS NOT NULL
      ORDER BY match_date ASC, id ASC
    `).all() as any[];

    const cleanRet = retPool.filter(m => !alreadyIngestedIds.has(m.id)).slice(0, 200);
    for (let i = 0; i < 200; i++) {
      const m = cleanRet[i];
      cohort.push({
        sourceName: 'sackmann',
        sourceMatchId: `p3_ret_base_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'retirements_base', phase: 'phase_3' },
      });
    ...
    for (let i = 0; i < 200; i++) {
      ...
      cohort.push({
        sourceName: 'pbp',
        sourceMatchId: `p3_ret_overlap_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'retirements_overlap', phase: 'phase_3' },
      });
      bucketCounts['5. Retirements & Walkovers (Overlaps)']++;
    }

    // -------------------------------------------------------------------------
    // BUCKET 6: Sibling Ambiguity & Ambiguous Aliases (400 matches)
    // -------------------------------------------------------------------------
    const siblingPool = sourceDb.prepare(`
      ...
      WHERE (
          ...
          OR winner_name LIKE '%Ruusuvuori%' OR loser_name LIKE '%Ruusuvuori%'
          OR winner_name LIKE '%Korda%' OR loser_name LIKE '%Korda%'
        )
        AND winner_name IS NOT NULL AND loser_name IS NOT NULL
      ORDER BY match_date ASC, id ASC
    `).all() as any[];

    const cleanSibling = siblingPool.filter(m => !alreadyIngestedIds.has(m.id)).slice(0, 400);
    for (let i = 0; i < 400; i++) {
      const m = cleanSibling[i];
      cohort.push({
        sourceName: 'sackmann',
        sourceMatchId: `p3_sibling_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'sibling_ambiguity', phase: 'phase_3' },
      });
      bucketCounts['6. Sibling & Ambiguous Aliases']++;
    }

    // -------------------------------------------------------------------------
    // BUCKET 7: Adversarial Identity Conflicts (400 matches: 200 base + 200 conflicts)
    // -------------------------------------------------------------------------
    const advPool = tourMatches.filter(m => !alreadyIngestedIds.has(m.id)).slice(3800, 4000);
    for (let i = 0; i < 200; i++) {
      const m = advPool[i];
      cohort.push({
        sourceName: 'sackmann',
        sourceMatchId: `p3_adv_base_${m.id}`,
        matchDate: m.match_date,
        tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
        gender: m.tour === 'WTA' ? 'F' : 'M',
        rawTournamentName: m.tourney_name,
        ...
        rawWinnerName: m.winner_name,
        rawScore: m.score,
        rawPayload: { original_id: m.id, bucket: 'adversarial_base', phase: 'phase_3' },
      });
      bucketCounts['7. Adversarial Identity Conflicts (Base)']++;
    }

    for (let i = 0; i < 200; i++) {
      const m = advPool[i];
      if (i < 100) {
        // Inverted winner conflict (VETO_WINNER_MISMATCH)
        cohort.push({
          sourceName: 'pbp',
          sourceMatchId: `p3_adv_winner_conflict_${m.id}`,
          matchDate: m.match_date,
          tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
          gender: m.tour === 'WTA' ? 'F' : 'M',
          rawTournamentName: m.tourney_name,
          ...
          rawPlayer1: m.winner_name,
          rawPlayer2: m.loser_name,
          rawWinnerName: m.loser_name, // conflicting winner
          rawScore: m.score,
          rawPayload: { original_id: m.id, bucket: 'adversarial_winner_conflict', phase: 'phase_3' },
        });
      } else {
        // Inverted score conflict (VETO_SCORE_INVERSION)
        cohort.push({
          sourceName: 'pbp',
          sourceMatchId: `p3_adv_score_conflict_${m.id}`,
          matchDate: m.match_date,
          tour: m.tour === 'WTA' ? 'WTA' : 'ATP',
          gender: m.tour === 'WTA' ? 'F' : 'M',
          rawTournamentName: m.tourney_name,
          ...
          rawPlayer2: m.loser_name,
          rawWinnerName: m.winner_name,
          rawScore: '0-6 0-6', // conflicting score
          rawPayload: { original_id: m.id, bucket: 'adversarial_score_conflict', phase: 'phase_3' },
        });
      }
      bucketCounts['7. Adversarial Identity Conflicts (Conflicts)']++;
    }
...
/**
 * Runs the Phase 3 large batch ingestion:
 * - Pre-phase snapshot creation (bak_pre_p3)
 * - Chunked transaction execution (20 x 500 matches)
 * - WAL passive checkpointing between chunks
 * - Immediate invocation of Phase Gate Validator
 */
export function runPhase3Ingestion(options: Phase3Options = {}): Phase3IngestionReport {
  ...
  if (targetDbPath === liveDbPath) {
    throw new Error('SECURITY VIOLATION: Refusing to run Phase 3 ingestion on live production database data/database.sqlite');
  }
  if (targetDbPath === legacyDryRunPath) {
    throw new Error('SAFETY VIOLATION: Refusing to run Phase 3 ingestion on legacy copy data/database.dryrun.sqlite');
  }
  ...
  const snapshotPath = `${targetDbPath}.bak_pre_p3`;
  ...
  const { cohort, bucketCounts } = buildPhase3Cohort(sourceDbPath, db);
  if (cohort.length !== 10000) {
    throw new Error(`Phase 3 cohort size mismatch: expected exactly 10000, got ${cohort.length}`);
  }

  const linker = new MatchLinkerEngine(db);
  const chunks: Phase3ChunkResult[] = [];

  let totalAutoLinks = 0;
  let totalNewCanonicals = 0;
  let totalReviewQueues = 0;

  // 3. Process cohort in 20 chunks of 500
