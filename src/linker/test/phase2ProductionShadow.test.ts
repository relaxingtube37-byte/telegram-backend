...
import { Phase2ProductionShadowRunner } from '../../scripts/runPhase2ProductionShadow';

console.log('======================================================');
console.log('   PHASE 2 PRODUCTION SHADOW PREREQUISITE TEST SUITE  ');
console.log('======================================================\n');
...
async function runTests() {
  ...
  console.log('[Test Suite 1: Dry-Run Safety Verification]');
  {
    ...
    assert(report.totalCohortSize === 2500, 'Cohort size is exactly 2,500 deterministic matches');
    assert(report.totalChunks === 50, 'Cohort is partitioned into exactly 50 chunks of 50 matches');
  ...
  {
    const runner = new Phase2ProductionShadowRunner({ isDryRun: true });
    const report = await runner.run();
  ...
  // Test 3: Sandbox Execution of the 50-Chunk Serialized Runner
  console.log('\n[Test Suite 3: Sandbox Execution & Sentinel Verification]');
  {
    const sandboxDir = path.resolve('data/scratch_test_phase2_shadow');
    if (!fs.existsSync(sandboxDir)) fs.mkdirSync(sandboxDir, { recursive: true });

    const sandboxDbPath = path.join(sandboxDir, 'test_phase2_shadow_db.sqlite');
    ...
    const v2CountBefore = (liveDb.prepare('SELECT count(1) as c FROM canonical_matches_v2').get() as any).c;
    liveDb.close();

    // Copy production DB to isolated sandbox
    fs.copyFileSync('data/database.sqlite', sandboxDbPath);

    const sandboxRunner = new Phase2ProductionShadowRunner({
    ...
    assert(execReport.chunksCompleted === 50, 'All 50 micro-chunks committed successfully');
    ...
    assert(snapFiles.length > 0 && snapFiles[0].includes('database_wal_safe_pre_phase2.sqlite'), 'Pre-phase snapshot created via native backup API');
  ...
  console.log('\n[Test Suite 4: Production Database Invariant]');
  {
    ...
    assert(v2Count === 300, 'Real production canonical_matches_v2 remains strictly at 300 (Phase 1 count, Phase 2 NOT executed on prod)');
  }
