...
import fs from 'fs';
import { runPhase2Ingestion } from '../phase2IngestionRunner';
...
async function runAllTests() {
  console.log('======================================================');
  console.log('   PHASE 2 MEDIUM BATCH INGESTION TEST SUITE          ');
  console.log('======================================================\n');
  ...
  await assertTest('Runner throws security violation if target is live production database', () => {
    try {
      runPhase2Ingestion({ targetDbPath: LIVE_DB_PATH });
  ...
  await assertTest('Runner throws safety violation if target is legacy dry-run copy', () => {
    try {
      runPhase2Ingestion({ targetDbPath: LEGACY_DRYRUN_PATH });
  ...
  await assertTest('Pre-phase snapshot is created and has valid SQLite format 3 header', () => {
    const snapPath = `${TARGET_DRYRUN_PATH}.bak_pre_p2`;
