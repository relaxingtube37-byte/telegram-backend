...
import { runPhase1Ingestion } from '../phase1IngestionRunner';
...
async function runAllTests() {
  console.log('======================================================');
  console.log('   PHASE 1 PILOT INGESTION TEST SUITE                 ');
  console.log('======================================================\n');
  ...
  await assertTest('Runner throws security violation if target is live production database', () => {
    try {
      runPhase1Ingestion({ targetDbPath: LIVE_DB_PATH });
  ...
  await assertTest('Runner throws safety violation if target is legacy dry-run copy', () => {
    try {
      runPhase1Ingestion({ targetDbPath: LEGACY_DRYRUN_PATH });
  ...
  await assertTest('Pre-phase snapshot is created and has valid SQLite format 3 header', () => {
    const snapPath = `${TARGET_DRYRUN_PATH}.bak_pre_p1`;
    // If it doesn't exist yet, create a test copy to verify snapshot logic
