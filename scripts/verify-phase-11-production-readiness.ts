...
async function main() {
  ...
  // P11-PRE-3: Catch-Up Ingestion Verification
  {
    ...
    const hasIngest = !!ingestEv;
    const ingestPass = hasIngest && ingestEv.status === 'PASS' && ingestEv.unmigrated_rows === 0;
    const catchUpNeeded = hasManifest && (delta!.catch_up_required as boolean) && !ingestPass;
    R({
      gate_id: 'P11-PRE-3', title: 'Catch-Up Ingestion Verification',
      status: !hasManifest ? 'BLOCKED' : (ingestPass ? 'PASS' : (catchUpNeeded ? 'PENDING_HUMAN_ACTION' : 'PASS')),
      measured_value: ingestPass
        ? `ingested=${ingestEv?.ingested_rows}, unmigrated=${ingestEv?.unmigrated_rows}, pass2_delta=${ingestEv?.pass2_idempotency_delta}`
        : (hasManifest ? `catch_up_required=${catchUpNeeded}` : 'NOT_AVAILABLE'),
      acceptance_threshold: 'Zero unmigrated delta rows in staging PG; idempotency pass 2 delta = +0',
      evidence_path: INGEST_PATH,
      blocking_reason: !hasManifest ? 'Phase B (P11-PRE-2) must complete first' : (ingestPass ? '' : 'Run: npx tsx scripts/ingest-render-delta.ts'),
  ...
  {
    const stagingBench = disarm && ((disarm.benchmark_verdict as string) === 'PASS' || disarm.passedSla === true);
    ...
    const p99Val = (disarm?.p99Ms ?? disarm?.p99_latency_ms) as number | undefined;
    const p99  = p99Val !== undefined ? p99Val.toFixed(4) + 'ms' : 'NOT_MEASURED';
    R({
      gate_id: 'P11-PRE-6', title: 'Render Runtime Disarm Benchmark',
      status: pass ? 'PASS' : (stagingBench ? 'PENDING_HUMAN_ACTION' : 'BLOCKED'),
      measured_value: `staging p99=${p99} (target_runtime=${disarm?.target_runtime ?? 'staging_environment'})`,
      ...
      blocking_reason: pass ? '' : (stagingBench ? 'Staging benchmark PASS (p99=' + p99 + ' < 10ms) but must re-run on Render container itself' : 'No benchmark recorded. Run verify-phase-11-canary-router.ts on Render container.'),
